...
export const schema = z.object({
  ...
  // Public base URL of the API itself, used to build the callback URLs that
  // external services (Meta webhooks, QStash scheduled jobs) call back into.
  // OPTIONAL: when unset, it is derived from BETTER_AUTH_URL by swapping :3000 for
  // :API_PORT (the legacy same-host assumption — see resolveApiPublicUrl). That
  // derivation only works when web and API share a host; a tunnel (cloudflared/
