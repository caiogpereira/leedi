...
- [ ] [Review][Patch] Impersonate route does not validate `tenantId` is a UUID — a non-UUID string passes the `length>0` check and throws on the `audit_logs.target_tenant_id` insert (500 instead of 400). [apps/admin/app/api/admin/impersonate/route.ts:333; packages/auth/src/use-cases/start-impersonation.ts:4330]
- [ ] [Review][Patch] `startImpersonation` never verifies the target tenant exists or belongs to the admin's workspace before setting cookies/writing the audit row — AC#1 scopes impersonation to "tenants in the workspace". Fix: load the tenant via the workspace-admin/service path and reject foreign/nonexistent tenants. [packages/auth/src/use-cases/start-impersonation.ts:4319-4338]
- [ ] [Review][Patch] Impersonation 1-hour expiry is enforced only by cookie `maxAge`, never re-validated server-side — `resolveImpersonation` honors the cookie without comparing `expiresAt`, so a refreshed/extended cookie outlives the window. Pitfall: "Do NOT allow impersonation to renew silently past 1 hour." [apps/dashboard/app/layout.tsx:856-875]
- [x] [Review][Defer] AC#2 (audit-on-mutation while impersonating) NOT implemented — `writeAuditLog` is exported but unwired; Task 4's Hono audit middleware in `apps/api` does not exist, so writes during impersonation are not recorded. Integration/E2E tests (Task 7) absent. — deferred (introduced by Epic 2; AC unmet, story cannot be "done") [packages/tenancy/src/use-cases/write-audit-log.ts]
- [x] [Review][Defer] No shared `requireWorkspaceAdmin` helper / no dashboard->admin redirect (Task 1) — the super_admin gate is inlined in the tenants page and impersonate route. — deferred (introduced by Epic 2) [apps/admin/app/(admin)/tenants/page.tsx]
- [x] [Review][Defer] `getWorkspaceAdmin` ignores `workspaceId` and `.limit(1)` picks an arbitrary row — audit-attributed `workspaceId` is nondeterministic if a staff member ever spans multiple workspaces (single-workspace assumption unguarded). — deferred (introduced by Epic 2) [packages/auth/src/workspace-guard.ts:3467-3475]
- [x] [Review][Defer] CSRF defense-in-depth (cross-cutting) — custom state-changing JSON routes (impersonate/switch/stop) rely solely on `SameSite=Lax`; no CSRF token or server-side Origin/Content-Type assertion. — deferred (introduced by Epic 2) [apps/dashboard/app/api/tenant/switch/route.ts]
