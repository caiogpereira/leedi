...
- [ ] Task 1: DB schema + migration (AC: #1)
  - [ ] Add `followups` table to `packages/db/src/schema/dispatch.ts`
  - [ ] Define `pgEnum('followup_status', ['agendado', 'enviado', 'cancelado', 'janela_fechada'])`
  - [ ] `followups`: `id` (uuid pk), `tenantId` (uuid FK), `leadId` (uuid FK → `leads.id`), `conversationWindowId` (uuid FK → `conversation_windows.id`), `agendadoPara` (timestamptz notNull), `motivo` (text notNull), `conteudoSugerido` (text nullable), `status` (followupStatusEnum notNull default `'agendado'`), `createdAt`
  - [ ] **Estratégia de migração**: se implementando 13.4 na mesma sessão que 13.2 e 13.3 (antes de qualquer `db push`), adicionar `followups` à migração 0012. Se 0012 já estiver aplicada, criar migração 0013 com `dispatch_rules` + `followups` (compartilhada com 13.3).
  - [ ] `ENABLE ROW LEVEL SECURITY`; tenant isolation policy; no `updated_at` (status updated in-place is OK)
  - [ ] Re-export from `packages/db/src/schema/index.ts`
- [ ] Task 2: `agendar_followup` agent tool (AC: #2, #7)
  - [ ] Create `apps/api/src/use-cases/agent/tools/agendar-followup.ts`
  - [ ] Input: `{ agendadoPara: string (ISO), motivo: string, conteudoSugerido?: string, leadId, tenantId, conversationWindowId }`
  - [ ] Validate tool is enabled in `agent_config.ferramentas_habilitadas` (AC: #7)
  - [ ] Validate `agendadoPara` ≤ `conversationWindow.started_at + 23h` (AC: #2)
  - [ ] Insert `followups` record
  - [ ] Enqueue BullMQ delayed job `send-followup` with `delay = agendadoPara - now()`, `jobId: followup-{followupId}`
  - [ ] Return success confirmation string for the agent to include in its response
  - [ ] Register tool in the agent tool dispatcher (Story 7.2) — add `agendar_followup` to the tool definitions list
- [ ] Task 3: `solicitar_reengajamento` agent tool (AC: #5)
  - [ ] Create `apps/api/src/use-cases/agent/tools/solicitar-reengajamento.ts`
  - [ ] Input: `{ motivo: string, leadId, tenantId }`
  - [ ] Query `dispatch_rules WHERE tenant_id = ? AND ativo = true` — pick first match (or most recently activated)
  - [ ] If found: enqueue `dispatch-recovery-target` with `delay: 0` (immediate)
  - [ ] If not found: return the "no active rule" error message (AC: #5)
  - [ ] Register in agent tool dispatcher
- [ ] Task 4: `send-followup` BullMQ job processor (AC: #3, #4, #6)
  - [ ] Create `apps/api/src/jobs/send-followup.ts`
  - [ ] Fetch `followups` record; if `status !== 'agendado'`, skip (idempotency)
  - [ ] Check lead conversion: if `lead.comprou = true` → set `followups.status = 'cancelado'`, return (AC: #6)
  - [ ] Check window: fetch `conversation_windows` record for `followups.conversation_window_id`; compute `windowCloseTime = started_at + 24h`
  - [ ] If `now() < windowCloseTime` (window open): send `conteudo_sugerido` (or default "Oi! Só passando para saber se ficou alguma dúvida 😊") via `connection.enviarTexto()`; set `followups.status = 'enviado'` (AC: #3)
  - [ ] If `now() >= windowCloseTime` (window closed): set `followups.status = 'janela_fechada'`; if matching active dispatch rule found, enqueue `dispatch-recovery-target` (AC: #4)
  - [ ] Register in BullMQ worker bootstrap
- [ ] Task 5: Agent tool registration (AC: #2, #5)
  - [ ] In `apps/api/src/use-cases/agent/tools/index.ts` (Story 7.2), add `agendar_followup` and `solicitar_reengajamento` tool definitions (name, description, input schema) to the tools array passed to the Claude Agent SDK
  - [ ] Add both tools to `agent_config.ferramentas_habilitadas` list (Story 7.1 UI)
- [ ] Task 6: Tests (AC: #2, #3, #4, #5, #6, #7)
  - [ ] Unit: `agendar_followup` rejects `agendadoPara` > 23h from now
  - [ ] Unit: `agendar_followup` rejects if tool disabled in agent config
  - [ ] Unit: `send-followup` job sends free message when window open
  - [ ] Unit: `send-followup` job sets `janela_fechada` when window closed, enqueues fallback
  - [ ] Unit: `send-followup` job sets `cancelado` when lead has converted
  - [ ] Unit: `solicitar_reengajamento` returns error message when no active rule found
  - [ ] Integration: full flow — agent calls `agendar_followup` → job fires → free WhatsApp message sent
