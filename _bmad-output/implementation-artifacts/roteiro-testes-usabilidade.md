...
| **F-42** | **J-22** | **Bug (MED) — FIXED (doc-grounded + unit-pinned; NOT live-validated)** | **The normalizer read `buyer.phone`, but Hotmart 2.0 purchase events carry the phone as `buyer.checkout_phone` (number) + `buyer.checkout_phone_code` (DDD) → leads created with an empty `telefone`** (confirmed live: a `compra_aprovada` lead got `telefone=''`). Cart-abandonment events use `buyer.phone`. **Fix (commit `6909260`):** build DDD+number from `checkout_phone_code`+`checkout_phone`, fall back to `buyer.phone`; downstream `normalizePhone` prepends `+55` (robust to code=DDD or code=country since it guards `startsWith('55')`). Grounded in Hotmart's official 2.0 docs (code `"31"` = DDD, phone `"999999999"` = number) — not inferred. +4 unit tests. **⚠️ Live validation used ONLY Hotmart's synthetic all-9s test data** (`checkout_phone_code='999999999'`, `checkout_phone='99999999900'` → produced the invalid `+99999999999999999900`); a **real-purchase phone has not been validated end-to-end** — confirm when a real (non-test) Hotmart purchase flows through. | **FIXED (doc-grounded + unit); real-phone e2e pending.** |
| **F-43** | **J-22** | **Bug (MED) — partial fix + audit deferred** | **Real Hotmart 2.0 `event` names diverge from `EVENT_MAP` (`packages/gateway/normalizers/hotmart.ts`).** Captured-and-fixed: cart abandonment = **`PURCHASE_OUT_OF_SHOPPING_CART`** (map had `CART_ABANDONED`) and subscription cancel = **`SUBSCRIPTION_CANCELLATION`** (map had `SUBSCRIPTION_CANCELED`) — both added (doc-verified) and **live-proven** to map (`carrinho_abandonado`/`assinatura_cancelada`). Confirmed-correct from capture: `PURCHASE_BILLET_PRINTED`→`boleto_gerado` (event [10], processed). **⚠️ UNVERIFIED — `PURCHASE_PIX_GENERATED` (the PIX recovery trigger) never appeared in the 16-event batch** → same shape as the `CART_ABANDONED` bug; if the real name differs, **PIX recovery is silently dead** (one of the 3 `dispatch_rule_trigger` recovery types). Long tail unverified: `PURCHASE_DELAYED`/`PURCHASE_EXPIRED` (no canonical), `SUBSCRIPTION_STARTED`/`SUBSCRIPTION_OVERDUE`/`PURCHASE_REFUSED` (guessed names, no canonical/wrong-name risk). **Fix (commit `6909260`):** added the 2 doc-verified names. **Defer:** verify the PIX event name specifically + a full event-taxonomy reconciliation against Hotmart's reference. | **Partial FIX; PIX-name + full taxonomy audit deferred (→ PL-20 cluster / deferred-work).** |
