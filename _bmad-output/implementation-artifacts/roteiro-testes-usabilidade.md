...
- **Tier:** 3 · **Driver:** Claude+Caio · **Status:** **done** (commit `6909260`, 2026-06-18) — proven end-to-end against **16 real Hotmart 2.0 test deliveries** through the tunnel. Headline **F-40 (CRITICAL, prod-dead)**: handler read the hottok only from `?hottok=` query, but Hotmart sends it in the **`X-HOTMART-HOTTOK` header** (16/16; query always empty) → every real delivery 401'd → gateway dead in prod. Fixed (header first, query fallback); 401→200 live. **F-39 sibling #1** (dedup `.rows` misread) live-proven: `gateway_events` rows now insert + dedup on replay (pre-fix `isDuplicate` threw before insert). **F-39 sibling #2** (recovery dispatch `.rows` misread) live-proven: fired `PURCHASE_OUT_OF_SHOPPING_CART` with a seeded active `dispatch_rules(trigger=carrinho_abandonado)` → QStash message **CREATED** to `gateway/dispatch-recovery-target` (pre-fix the `.rows.length` threw → never published). Full pipe: webhook → `process-event` → `handlePurchaseApproved`/`handleRecoveryEvent` → lead + journey event, multiple event types processed (`compra_aprovada`/`carrinho_abandonado`/`chargeback`/`compra_cancelada`/`compra_reembolsada`/`boleto_gerado`/`assinatura_cancelada`). Also fixed **F-42** (phone field) + **F-43** (real 2.0 event names). **Epic 11 concurrent-dup gap CONFIRMED live** (the 16-event batch fires ~simultaneously → same `transaction` stored multiple times; SELECT-then-INSERT, no unique index). Surfaced **F-41 → PL** (no UI field for the client's HOTTOK). DB restored to clean state; integration row (held the real account HOTTOK) deleted.
- **Steps:** fire a Hotmart purchase webhook → confirm signature verification → lead status updated → recovery event flow.
- **Observe:** idempotency (deferred-work Epic 11 — app-layer SELECT-then-INSERT, no unique index; concurrent dup possible — **confirmed live**); recovery event published *after* commit (Epic 11 fix — verified: journey written in the critical tx, dispatch published after).
- **Verified facts for re-tests (don't re-discover):** mount path is **`/webhooks/hotmart/:webhookUrlPath`** (root, NOT `/api/webhooks/hotmart` — the old runbook outline was wrong; a real delivery to `/api/webhooks/hotmart` 404'd). Hottok arrives in header `X-HOTMART-HOTTOK` (+ inconsistently top-level body `hottok`, 1/16). Purchase events: phone in `buyer.checkout_phone`+`checkout_phone_code` (DDD); cart events: `buyer.phone`. Real cart event = `PURCHASE_OUT_OF_SHOPPING_CART`; sub-cancel = `SUBSCRIPTION_CANCELLATION`. The Hotmart **real-account** "disparar eventos de teste" button (synthetic data) is the cheapest real delivery — sandbox console has no fire-test option.
...
- **Tier:** X-cut (VAPID keys, no tunnel) · **Driver:** Claude+Caio · **Status:** done (F-44; commit `e8dc60d`)
...
| **F-40** | **J-22** | **Bug (CRITICAL) — FIXED + live-proven** | **The Hotmart webhook read the hottok only from the `?hottok=` query param; Hotmart 2.0 sends it in the `X-HOTMART-HOTTOK` header → every real delivery 401'd → gateway dead in prod.** Proven against **16 real Hotmart 2.0 test deliveries** through the tunnel (`Jodd HTTP` UA): all 16 had `query={}`, all 16 had header `x-hotmart-hottok=<account HOTTOK>` (+ a top-level body `hottok` on only 1/16, so the body field is unreliable). The handler (`webhooks/hotmart.ts:18`) did `c.req.query('hottok')` → always `undefined` → `401`. Same class as F-37 (real-format vs assumed-format). **Root-caused by top-of-handler instrumentation** (F-37 / [[feedback_debug_at_arrival_point]] playbook; reverted after). **Fix (commit `6909260`):** read `c.req.header('x-hotmart-hottok')` first, query as fallback. Live-verified: same delivery 401→**200**, then full pipe ran (gateway_events insert, QStash `process-event`, lead+journey). Also nailed the **mount path**: `/webhooks/hotmart/:path` (root) — a real delivery to `/api/webhooks/hotmart` (old runbook-3 outline) **404'd**. +2 regression tests (header accept / header-wrong 401, no query fallback). | **FIXED + live-proven.** |
| **F-41** | **J-22** | **Bug (HIGH) → PL** | **No UI for the client to enter their Hotmart HOTTOK, and `createGatewayIntegration` mints a random-UUID `webhookSecret` → the per-tenant secret can never equal the value Hotmart actually sends.** The handler validates `hottok === gateway_integrations.webhookSecret`, but `create-gateway-integration.ts:22` sets `webhookSecret = randomUUID()`. Hotmart sends the **account's own fixed HOTTOK** (e.g. `ExvY…`), which Leedi never learns — the onboarding gateway step (`/onboarding` step 3) shows the generated **URL** but has **no field** to paste the client's HOTTOK. So in prod, with F-40 fixed, every real client's deliveries still 401. (User intuited this: *"a interface não oferece nada para adicionarmos as credenciais"*.) **Fix (design, launch-gating for Hotmart):** add a HOTTOK input to the onboarding gateway step and store it as `webhookSecret`. Local test was unblocked by manually setting the secret = the account HOTTOK. | **→ PL-20 (launch-gate, Hotmart).** Logged; not built (feature work). |
| **F-42** | **J-22** | **Bug (MED) — FIXED + live-proven (real phone)** | **The normalizer read `buyer.phone`, but Hotmart 2.0 purchase events carry the phone as `buyer.checkout_phone` + `buyer.checkout_phone_code` (DDD) → leads created with an empty `telefone`** (confirmed live: a `compra_aprovada` lead got `telefone=''`). Cart-abandonment events use `buyer.phone`. **First fix (commit `6909260`) was WRONG** — it concatenated `code`+`checkout_phone`; a real checkout (2nd account) revealed `checkout_phone` **already includes the DDD** (`'35999731201'` with code `'35'`) → concatenation would double it to `+3535…`. **Corrected fix:** use `checkout_phone` as-is when it has ≥10 digits (DDD present), only prepend the code when it's DDD-less (<10, the shape in Hotmart's docs example); fall back to `buyer.phone`; downstream `normalizePhone` adds `+55`. **Live-proven with REAL purchase data:** leads created with correct `+5535999731201` (Caio) and `+5535991923321` (Viviane). +unit tests incl. the real-data no-double case. | **FIXED + live-proven (real phone).** |
| **F-43** | **J-22** | **Bug (MED) — FIXED (incl. PIX) + tail audit deferred** | **Real Hotmart 2.0 `event` names diverge from `EVENT_MAP`.** Fixed + **live-proven**: cart abandonment = **`PURCHASE_OUT_OF_SHOPPING_CART`** (map had `CART_ABANDONED`), sub-cancel = **`SUBSCRIPTION_CANCELLATION`** (map had `SUBSCRIPTION_CANCELED`), billet = `PURCHASE_BILLET_PRINTED`→`boleto_gerado` (confirmed in capture). **PIX RESOLVED (Hotmart support + real PIX checkout):** there is **no dedicated PIX event** — Hotmart reuses **`PURCHASE_BILLET_PRINTED` with `purchase.payment.type='PIX'`** (the real PIX delivery carried `status: 'BILLET_PRINTED'`, **NOT** the `WAITING_PAYMENT` the support first suggested — empirical capture corrected the support). **Fix:** the normalizer reclassifies a PIX-typed billet-printed event to **`pix_gerado`** (content-based, not event-name). **Live-proven:** replayed the real PIX body → `gateway_events.evento_canonico='pix_gerado'` + journey `pix_gerado`; then the **real PIX payment** fired `compra_aprovada`→`comprou`. **Tail still unverified (deferred):** `PURCHASE_DELAYED`/`PURCHASE_EXPIRED` (no canonical), `SUBSCRIPTION_STARTED`/`SUBSCRIPTION_OVERDUE`/`PURCHASE_REFUSED` (guessed names). **Fixes in commits `6909260` + follow-up.** | **FIXED incl. PIX (live-proven); tail audit deferred.** |
| **F-44** | **J-23** | **Bug (HIGH) — FIXED + verified end-to-end** | **Push registration no-oped silently: `NEXT_PUBLIC_VAPID_PUBLIC_KEY` was `undefined` in the client bundle, so `registerPushSubscription` (`apps/dashboard/src/lib/push-registration.ts:11`) returned before registering.** Next.js only auto-loads `.env` from the app's own dir; the monorepo-root `.env` (where the VAPID vars live) was never read at the point Next inlines `NEXT_PUBLIC_*` into the client bundle. **Fix (commit `e8dc60d`):** `apps/dashboard/next.config.ts` `process.loadEnvFile('../../.env')` before client compilation; added `NEXT_PUBLIC_VAPID_PUBLIC_KEY` (= `VAPID_PUBLIC_KEY`) to `.env`/`.env.example`. **Verified end-to-end:** (a) inlining proven autonomously — a clean Playwright Chromium with a minted Better-Auth session registered `/sw.js` (the code returns *before* `register()` if the key is falsy, so SW-present ⟹ key inlined); (b) subscription persists through proxy→API→DB (real FCM endpoint, `p256dh` len 87); (c) per-endpoint web-push diagnostic returned **FCM 201** for the real browser's endpoint; (d) the real browser **received the toast**. **Environment gotcha (not Leedi code):** initial non-receipt was Windows having **Chrome notifications disabled** (Settings → System → Notifications) — `showNotification` ran but the OS suppressed display; re-enabling fixed it. Also note: a SW's `console.log` surfaces in the **SW's own inspector**, not the page F12 console — "nothing in F12" is not evidence the SW didn't fire. | **FIXED + verified end-to-end** (real browser received). |
...
- ~~**`/relatorios` sidebar link** (J-10)~~ — **RESOLVED (F-23).** The dead `/relatorios` nav item was removed from `Sidebar.tsx`; analytics live on the home dashboard (`/`); a dedicated page is deferred to future work.
