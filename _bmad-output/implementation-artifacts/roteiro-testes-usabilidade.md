...
| F-29 | J-12 / global | **Bug (MED) — user-reported** | **No logout button anywhere** — neither the super-admin panel (`:3002`) nor the user dashboard (`:3001`) exposes a way to sign out. Sessions persist with no UI to end them (problematic on shared devices; also blocks role-switching without clearing cookies manually). | Open — add a logout action (calls Better-Auth signOut) to both shells' user menus. |
