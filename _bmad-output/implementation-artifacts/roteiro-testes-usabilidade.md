...
- **Tier:** 0 (steps 1, 3–5; step 2 = WhatsApp connect is **Tier 1**, see J-13) · **Driver:** Claude+Caio · **Status:** BLOCKED on F-31 — self-serve signup never provisions a tenant, so a fresh account has no workspace to onboard ("Nenhum workspace encontrado" on both `/` and `/onboarding`). Untestable until F-31 is fixed.
...
| F-28 | J-12 | **Bug (MED)** | **Admin app redirects unauthorized/unauthenticated to a non-existent `/login` → 404.** `apps/admin/app/(shell)/layout.tsx` does `redirect("/login")` for both no-session and non-super-admin, but the admin app has no `/login` page (and no middleware) → `:3002/login` 404s. A `/403` page exists and the web login lives on `:3000`; the redirect should target one of those. (The *guard* itself works — it does block the owner — but the landing is a 404.) | Open — repoint to `/403` or `${BETTER_AUTH_URL}/login`. |
| F-29 | J-12 / J-01 / global | **Bug (MED) — user-reported** | **Logout is missing or broken everywhere.** The dashboard (`:3001`) and super-admin (`:3002`) shells expose **no** sign-out affordance. The onboarding shell *does* have a "Sair" link — but it points to `/api/auth/sign-out` **relative**, i.e. `:3001/api/auth/sign-out`, which is a **404** (the Better-Auth handler lives on the web origin `:3000`, and sign-out is a POST, not a GET page). So there is no working way to log out from any app. Add a sign-out action (Better-Auth `signOut`, POST to `${BETTER_AUTH_URL}/api/auth/sign-out` or a server action) to all shell user menus, and fix the onboarding link. | Open — add working logout to all shells. |
...
| **F-31** | **J-02 (+J-01)** | **Bug (CRITICAL) → launch-blocker** | **Self-serve signup never provisions a tenant — a new customer is permanently stuck.** `registerUser` (`packages/auth/src/use-cases/register-user.ts`) only calls `auth.api.signUpEmail` → creates the `users`+`accounts` rows but **no workspace, tenant, or membership**. There is no `databaseHooks` after-create in `auth.ts` and no self-serve provisioning use-case (`createTenant` is the *super-admin* flow: needs an existing `workspaceId` + invites the owner by email — wrong shape for self-signup). Verified end-to-end: registered `caiog.pereira+leedi-j01@gmail.com` → real Resend email (from `noreply@leedi.digital` ✅) → verified (`email_verified=true`) → login → dashboard AND `/onboarding` both show **"Nenhum workspace encontrado"**; `memberships` for the user = **0 rows**. So **J-02 (onboarding wizard) is untestable** — there's no trial tenant to onboard. Epic 19's "tenant default 'trial' → AC#1" was almost certainly mock-verified (fake-green). **Fix (design needed):** add self-serve provisioning (new workspace + `status:'trial'` tenant + `owner` membership) on signup — via a better-auth `databaseHooks.user.create.after` or synchronously in `registerUser` after `signUpEmail` (decide: provision at signup vs after email-verification to avoid orphan tenants for unverified emails). | **Open — CRITICAL. Blocks J-02 + every new customer. Caio: build self-serve provisioning (recommend after-verification hook).** |
