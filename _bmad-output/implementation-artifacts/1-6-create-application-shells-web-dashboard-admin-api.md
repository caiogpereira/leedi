...
- [ ] Task 2: Configure dev ports (AC: #1)
  - [ ] Set dev scripts: `web` → `next dev -p 3000`, `dashboard` → `next dev -p 3001`, `admin` → `next dev -p 3002`
...
- [ ] Task 3: Configure next-intl for pt-BR (AC: #1)
  ...
  - [ ] Create a minimal `messages/pt-BR.json` per app and render one localized string on the index page (no hardcoded UI strings — Architecture rule)
...
- App roles (Architecture 4): `web` = landing + login/signup; `dashboard` = tenant panel; `admin` = super-admin panel; `api` = Hono backend for webhooks/agent/jobs. Only shells + health here — NO auth, NO business routes (those come in later epics).
...
- next-intl with pt-BR default and no hardcoded UI strings is an Architecture rule — even the placeholder index text goes through a messages file.
...
- Do NOT scaffold `apps/api` as a Next.js route handler — it must be a standalone Hono server (Architecture explicitly separates the backend).
- Do NOT register routes before importing `@leedi/config` — env validation must fail-fast first (Story 1.3 AC). Put the config import as the first line of the entry.
