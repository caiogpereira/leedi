 ...
 * the session server-side and `redirect('/login')` for anyone who is not a
 * `super_admin`. We assert the anonymous redirect at the response level
 * (maxRedirects: 0). The `/403` page renders without auth, so we run a real axe
 * sweep against it as the Phase-1 a11y gate.
 *
