...
export async function handleRecoveryEvent(input: HandleRecoveryEventInput): Promise<void> {
  ...
  await withTenant(tenantId, async (tx) => {
    ...
    // Try to find and enqueue a dispatch rule (Story 13.3 dependency)
    // Wrapped in try/catch — dispatch_rules table may not exist yet
    try {
      ...
      // dispatch_rules table not yet created (Story 13.3) — skip silently
    }
  });
}
